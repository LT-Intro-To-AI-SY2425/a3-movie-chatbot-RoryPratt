...
def title_by_year_range(matches: List[str]) -> List[str]:
    ...
    """
    return [get_title(i) for i in movie_db if get_year(i) >= int(matches[0]) and int(matches[1]) >= get_year(i)]
...
def actors_by_title(matches: List[str]) -> List[str]:
    ...
    """
    ans = [get_actors(i) for i in movie_db if get_title(i) == matches[0]]
...
def search_pa_list(src: List[str]) -> List[str]:
    ...
    t = False
    if len(src) == 3:
        if src[:2] == ['who', 'directed']:
            ans = director_by_title([src[2]])
            t = True
    if len(src) == 6:
